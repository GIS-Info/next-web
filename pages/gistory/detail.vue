...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      // Extract title + issue label for the header
      this.cleanTitle = this.extractCleanTitle(raw);
      this.issueLabel = this.extractIssueLabel(this.fileName);

      // Strip the front title line so it doesn't render twice
      const stripped = this.stripFrontTitle(raw);

      // Clean image URLs (R2 proxy + relative path resolution), then render
      const cleaned = this.formatMarkdownImages(stripped);
      this.renderedContent = marked.parse(cleaned, { breaks: true, gfm: true });
    } catch (err) {
  ...
  methods: {
    /**
     * Rewrite image URLs in raw markdown:
     *   - `./images/x.png` → absolute GitHub raw URL
     *   - `*.r2.dev/...`   → proxied through wsrv.nl
     * Handles BOTH `![alt](url)` and `<img src="url">` forms.
     */
    ...
    /** Pull "title:" or "# " from markdown, strip "GIStory Issue N | " prefix */
    extractCleanTitle(content) {
    ...
    /** "gistory13_cyh.md" → "Issue 13" */
    extractIssueLabel(filename) {
      const num = parseInt(filename.match(/\d+/)?.[0] || 0, 10);
      return num ? `Issue ${String(num).padStart(2, '0')}` : 'Issue';
    },
    /** Remove the first "title:" / "# " line so we don't render the title twice */
...
/* Subtle ambient tint (matches list page) */
.markdown-detail::before {
  content: '';
  position: fixed;
  inset: 0;
  pointer-events: none;
...
/* ================================
   ...
   (uses :deep() because v-html bypasses scoped styles)
================================ */
.article-body {
  font-family: var(--sans);
  font-size: 17px;
...
/* Images — the main reason for the cleaning logic */
